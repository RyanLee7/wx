...
public abstract class SDcardSQLiteOpenHelper {
	...
	public SDcardSQLiteOpenHelper(Context context, String name,
			CursorFactory factory, int version,
			DatabaseErrorHandler errorHandler, String path) {
		if (version < 1)
			throw new IllegalArgumentException("Version must be >= 1, was "
					+ version);
	...
	private File getDatabasePath(String name) {
		...
		if (Environment.getExternalStorageState().equals(
			...
			// sdcard不存在时
			return mContext.getDatabasePath(mName);
		}
		...
		try {
			...
			if (db != null) {
				...
				try {
					 ...
					 * path = mContext.getDatabasePath(mName) .getPath(); db =
					 * SQLiteDatabase.openDatabase(path, mFactory,
					 * SQLiteDatabase.OPEN_READONLY, mErrorHandler); } else { db
					 * = mContext .openOrCreateDatabase( mName,
					 * mEnableWriteAheadLogging ?
					 * Context.MODE_ENABLE_WRITE_AHEAD_LOGGING : 0, mFactory,
					 * mErrorHandler); }
					 */
					...
					// 如果路径没有定义，使用系统自带的路径。
					if (path.length() == 0) {
						dbpath = mContext.getDatabasePath(mName).getPath();
					} else {
						dbpath = getDatabasePath(mName).getPath();
					}

					db = mContext
							.openOrCreateDatabase(
									dbpath,
									mEnableWriteAheadLogging ? Context.MODE_ENABLE_WRITE_AHEAD_LOGGING
											: 0, mFactory, mErrorHandler);
					...
					final String path = mContext.getDatabasePath(mName)
							.getPath();
